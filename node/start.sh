...
mkdir -p /data/blog/hexo && cd /data/blog/hexo && git init & git remote add origin $Hexo_backup_url  & expect -f /root/expect_git.sh


#cd /data/blog/hexo_blog/source/_posts && git init && git remote add origin $Hexo_note_url  && yes yes | git pull origin master
cd /data/blog/hexo_blog/source/_posts && git init && git remote add origin $Hexo_note_url expect -f /root/
